  ...
  for (const r of rows) {
    lines.push(
      [
        ...
        csvEscape(r.outcome),
        csvEscape(r.outcome_at),
        csvEscape(r.outcome_note),
...
router.get("/reports/appointments-json", async (req, res) => {
  const db = getPool();
  const from = req.query.dateFrom ? String(req.query.dateFrom).slice(0, 10) : null;
  const to = req.query.dateTo ? String(req.query.dateTo).slice(0, 10) : null;
  let sql = `
    SELECT a.id, a.booking_code, a.appointment_date, a.appointment_time, a.status, a.service_type,
           a.year_level, a.college, a.reason,
           a.outcome, a.outcome_at, a.outcome_note,
           a.session_duration_minutes,
           a.student_cancellation_reason,
  ...
  res.json({
    ...
    appointments: rows.map((r) => ({
      ...
      appointmentTime: String(r.appointment_time).slice(0, 8),
      status: r.status,
      ...
      studentCancellationReason: r.student_cancellation_reason,
      studentName: r.student_name,
      studentEmail: r.student_email,
      counselorName: r.counselor_name,
      counselorEmail: r.counselor_email,
